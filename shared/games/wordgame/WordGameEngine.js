...
export class WordGameEngine extends BaseGameEngine {
	/**
	...
	constructor(playerIds, settings = {}) {
		...
		super(uniquePlayerIds, settings);
	...
	serializeForPlayer(viewerId) {
		...
		const opponentChosenWord =
			isRevealPhase && this.wordsForGuesser[viewerId] ?
				this.wordsForGuesser[viewerId]
			:	null;

		const opponentChosenChampionId =
			isRevealPhase && this.championIdsForGuesser[viewerId] ?
				this.championIdsForGuesser[viewerId]
			:	null;
