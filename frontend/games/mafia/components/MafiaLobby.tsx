...
import { Check, Mic, Bot, Castle, UserX } from "lucide-react";
  ...
  return (
    <div className="relative isolate mx-auto my-6 max-w-2xl px-4 py-6 sm:px-6">
      ...
      <GameLobbyCore
        ...
        starting={starting}
        canStartOverride={canStart && lobbySetup.valid}
        roomCodeLabel="Chamber code"
        playersLabel={`${connectedCount} of ${lobby.maxPlayers} in room · ${gameplayCount} receive roles`}
              ...
              <MafiaCard variant="glass" interactive={false} className="mb-3.5">
                <MafiaCardContent className={clsx("space-y-3.5", lobbyCardBodyClass)}>
                  ...
                  <MafiaCard variant="inset" className="mt-1 space-y-1 p-2.5" aria-live="polite">
                    ...
                    {(lobbySetup.errors.length > 0 || lobbySetup.warnings.length > 0) && (
                      <ul className="m-0 list-none space-y-0.5 p-0 text-[0.68rem] leading-snug">
                        {lobbySetup.errors.map((msg) => (
                          <li
                            key={msg}
                            className="text-rose-200/95 before:mr-1 before:content-['•']"
                          >
                            {msg}
                          </li>
                        ))}
                    ...
                    {activeRoleSummary.length > 0 && (
                      <p
                        className={clsx(
                          "m-0 border-t border-amber-900/25 pt-1 text-[0.68rem] leading-snug",
                          lobbyBodyClass,
                        )}
                      >
        ...
        footerSlot={
          isHost && !lobbySetup.valid && lobbySetup.errors[0] ?
            <p className="text-xs text-rose-300/80 text-center mb-4">
              {lobbySetup.errors[0]}
            </p>
          : null
        }
